...
def get_final_test(user: User, db: Session) -> list[dict]:
    ...
    if not user.first_name or not user.last_name or not user.father_name:
        raise HTTPException(
            status_code=403,
            detail="Please fill in your full name (first name, last name, father's name) before taking the final test"
        )
...
def submit_final_test(user: User, answers: list, db: Session) -> dict:
    ...
    if passed:
        ...
        if not existing_cert:
            ...
            full_name = f"{user.last_name} {user.first_name} {user.father_name}".strip()
            if not full_name:
                full_name = user.name or "Foydalanuvchi"
